...
# 加载Whisper Small模型（适配Python 3.11）
@st.cache_resource
...
# ---------------------- 主界面 ----------------------
def main():
    st.title("🎙️ 智能音频转字幕工具（终极稳定版）")
    st.markdown("### 支持MP3/WAV、多语言识别、双语字幕、SRT导出")
    st.divider()
    ...
    if audio_file:
        # 加载模型
        with st.spinner("🔧 加载识别模型（首次需1分钟）..."):
            model = load_whisper_model()
        
        # 创建临时文件（用系统临时目录，避免权限问题）
        ...
        # 识别+翻译（核心步骤）
        with st.spinner(f"🎧 识别中（音频越长越久）..."):
            # 语言映射
            lang_map = {"中文":"zh","英文":"en","日语":"ja","韩语":"ko","法语":"fr","西班牙语":"es"}
            # 调用Whisper（最基础的方式，避免兼容性问题）
        ...
        for idx, seg in enumerate(target_segments):
            ...
            with col1:
                start = st.text_input(f"开始时间（秒）", f"{seg['start']:.2f}", key=f"s_{idx}")
            with col2:
                end = st.text_input(f"结束时间（秒）", f"{seg['end']:.2f}", key=f"e_{idx}")
            with col3:
                text = st.text_input(f"字幕 {idx+1}", remove_punctuation(seg["text"]), key=f"t_{idx}")
            ...
            try:
                ...
                end_float = seg["end"]
                st.warning(f"第{idx+1}行时间错误，已恢复默认")
        ...
        st.download_button(
            ...
            data=srt_content,
            file_name=f"{audio_file.name.split('.')[0]}_{target_language}.srt",
            mime="text/plain"
        )

        # 预览
        st.subheader("👀 字幕预览")
        st.text_area("SRT内容", srt_content, height=300)
